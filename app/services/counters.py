...
async def count_unique_view(request: Request, response: Response, post_id: str) -> None:
    """
    Nalicz 1 view / visitor / dzień.
    visitor_id dostarczany przez VisitorIdMiddleware jako request.state.visitor_id
    """
    visitor_id = getattr(request.state, "visitor_id", None)
    if not visitor_id:
        # fallback - normalnie nie powinno się zdarzyć jeśli middleware jest wpięty
        return
...
async def sync_comments_total(post_id: str) -> int:
    ...
    """
    cnt = await count_comments(post_id)
    await update_comments_total(post_id, cnt)
    return cnt
